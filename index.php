...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    // Validação se algum campo obrogatório estiver vazio Exiba mensagem
    ...
    $_FILES['imagem']['error'] === UPLOAD_ERR_OK){
        ...
        if(empty($erro)){
            ...
            // Salva os valores no bloco de notas
...
if(is_file($arquivo)){
    ...
    // Percorre cada linha e quebramos por | em difentes elementos
    foreach($linhas as $linha){
        // Explode na |
        $p = explode('|',$linha);
        // Monta os pedaços de forma organizada
        $eventos[] = [
            'titulo' => $p[0],
            'data' => $p[1],
            'hora' => $p[2],
